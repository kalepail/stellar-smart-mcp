...
export class MyMCP extends McpAgent<Env, unknown, Props> {
	...
	async init() {
		...
		const contractIds = await stub.getContracts(this.props.login);
					...
					async (params) => {
						try {
							...
							if (at.isReadCall) {
								...
								const address_sk = await stub.getAddress(this.props.login);
		...
		this.server.tool(
			...
			async ({ name, address }) => {
				...
				await stub.addContract(this.props.login, name, address);
		...
		this.server.tool(
			...
			{},
			async () => {
				const contracts = await stub.getContracts(this.props.login);
		...
		this.server.tool(
			...
			async ({ address }) => {
				stub.removeContract(this.props.login, address);
		...
		this.server.tool(
			...
			async ({ wallet }) => {
				stub.setAddress(this.props.login, wallet);
